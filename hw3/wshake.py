...
# data11 = loadmat('data/cam1_2.mat')
# data12 = loadmat('data/cam2_2.mat')
# data13 = loadmat('data/cam3_2.mat')

# ### Turn into numpy arrays using list comprehension
# dmtrx1 = np.array([[val for val in elements]
#                    for elements in data11['vidFrames1_2']])
# dmtrx2 = np.array([[val for val in elements]
#                    for elements in data12['vidFrames2_2']])
# dmtrx3 = np.array([[val for val in elements]
#                    for elements in data13['vidFrames3_2']])
...
# ### Save
# with open('data/shake1.npy', 'wb') as f:
#     np.save(f, dmtrx1g)

# with open('data/shake2.npy', 'wb') as f:
#     np.save(f, dmtrx2g)

# with open('data/shake3.npy', 'wb') as f:
#     np.save(f, dmtrx3g)

### Load
ta = time.time()
with open('data/shake1.npy', 'rb') as f:
    dm1 = np.load(f)

with open('data/shake2.npy', 'rb') as f:
    dm2 = np.load(f)

with open('data/shake3.npy', 'rb') as f:
    dm3 = np.load(f)

print('numpy time is ' + str(time.time() - ta))

### Synchronize them... (so annoying...)
# first clip
s1 = 13
e1 = dm1.shape[2]
m1 = dm1[:,:,s1:e1]
...
plt.title('Principal modes with camera shake')
